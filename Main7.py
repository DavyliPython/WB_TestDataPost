...
class testDataPost(QMainWindow, UI.Ui_MainWindow):
    def __init__(self):
        ...
        super().__init__()
        self.colorDex = ['#9966FF', '#990033', '#0066CC', '#FFCC00', '#FF6600', '#009933', '#FFCCFF', '#FF3333', '#FF00FF', '#996633' ]
        ...
        # self.actionA.triggered.connect(self.chartPlot)
    ...
    def chartPlot(self, currentItem, selectedItems):
        ...
        if self.chartPlotItems:
            ...
            #self.dataPlot.removeItem(selectedItems[0])
            self.dataPlot.removeItem(self.first_curve)

        self.updateViews()

        # print('1')
        # self.dataPlot.plotItem.scene().clear()
        # print('2')
        self.chartPlotItems = []
        self.chartVBs = []
        self.chartAxials = []
        ...
        aaa = self.dataSummary[self.item_Selected]
        #print(self.dataSummary[self.item_Selected])
        #print([float(x) for x in self.dataSummary[self.item_Selected]])
    ...
    def CI_Plot(self, currentItem):
        ...
        temp_label = currentItem.text(2) + '(' + currentItem.text(3) + ')'


        temp.setLabel('left', '<span style="font-size: 10pt"><span style="color: %s"> %s </span>' % (self.colorDex[0], temp_label))

        temp.setLabel('bottom', 'Test Data Point', units=' Points', **{'font-size': '12pt'})
        temp.getAxis('bottom').setPen(pyqtgraph.mkPen(color='#000000', width=1))
        # temp.showAxis('right')
        temp_2_plot = self.dataSummary[int(currentItem.text(0))]
        # print(temp_2_plot)
        pen1 = pyqtgraph.mkPen(color=self.colorDex[0]) #, width=2)
    ...
    def SI_Plot(self, currentItem, selectedItems):
        ...
        for i in range(0, len(selectedItems)):
            ...
            if selectedItems[i] != currentItem:
                ...
                temp_label = selectedItems[i].text(2) + '(' + selectedItems[i].text(3) + ')'
                ax_temp.setLabel('<span style="font-size: 10pt"><span style="color: %s"> %s </span>' % (self.colorDex[i+1], temp_label))
                # print('4')
                temp_2_plot = self.dataSummary[int(selectedItems[i].text(0))]
                # print(temp_2_plot[:100])
                temp_plotcurve = pyqtgraph.PlotCurveItem([float(x) for x in temp_2_plot], pen=self.colorDex[i+1])
    ...
    def chartPlot1(self):
        ...
        self.dataPlot.plot(range(self.dataSummary[self.item_Selected].__len__()),
                           [float(x) for x in self.dataSummary[self.item_Selected]], pen=pen1, clear=True)
        self.dataPlotRange.plot(range(self.dataSummary[self.item_Selected].__len__()),
                                [float(x) for x in self.dataSummary[self.item_Selected]], pen=pen2, clear=True)
        self.dataPlotRange.setZValue(1)
        ...
        # print('current item is %s', current_item)
        # print('selected items is %s', selected_items[0], selected_items[1], selected_items[2])
    ...
    def mouseMove(self, evt):
        ...
        if self.dataPlot.plotItem.sceneBoundingRect().contains(pos):
            ...
            if index > 0 and index < len(self.dataSummary[self.item_Selected]):
                ...
                label_text = "<span style='font-size: 10pt'> <span style='color :" + self.colorDex[0] + "'> " + "Test Point = " + str(round(mousePoint.x(),0)) + " Time : " + self.dataSummary[0][index] + "  " + str(currentItem.text(2)) + " = " + str(round(float(self.dataSummary[int(currentItem.text(0))][index]),2))

                for i in range(0, len(selectedItems)):
                    pass
                    if selectedItems[i] != currentItem:
                        label_text = label_text + "<span style='font-size: 10pt'> <span style='color :" + self.colorDex[i+1] + "'> "  + str(selectedItems[i].text(2)) + " = " + str(round(float(self.dataSummary[int(selectedItems[i].text(0))][index]),2))
                        #print(label_text)
    ...
    def regionUpdate(self):
        # print('region is updating')
        # self.region.setZValve(10)
        # print('region update done')
